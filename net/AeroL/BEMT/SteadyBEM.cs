...
namespace OpenWECD.AeroL.BEM.SteadyBEMT
{
    public class SteadyBEM :AeroL_IO_Subs. AL_PutBemEle
    {
        #region 字段区域
        ...
        {
            LogHelper.WriteLog("Run SteaddyBEM Solve！", title: "[success]");
            aerol = aerols;
            BlSpn = aerol.Geometry[0].BlSpn + aerol.HubRad;
            nb_Load = aerol.NumBldNds;
            Bldnum = aerol.Bldnum;
            //初始化初始化流
            AL_PutINI(aerol);
            PutInI(aerols.AerolData[1]);
        }
        ...
        /// Ct 总的推力系数
        /// </returns>
        private (double Cp, double Ct) BEM(T_ALAeroBladeElement[,] BlDEle, Airfoil1 Airfoils, double λ, double windspeed, double HubRad, double pitch = 0.0, double rho = 1.225, int BladeNum = 3)
        {
            ...
            for (int i = 0; i < Bldnum; i++)
            {
                ...
                {
                    double theta = BlDEle[j, i].STwist + pitch;
                    double Solid = BlDEle[j, i].SSolid / BlDEle[j, i].SBlspan;
                    (a1, b1, φ, AOA, cn, Ct, n, cl, cd, cm) = CalcOutputS(λ, theta, BlDEle[j, i].SFoilNum, Airfoils, TipRad, HubRad, BlDEle[j, i].SBlspan, Solid, a1, b1, BladeNum);
                    if (n <= 1998)
                    {
                        Vlocal = v * (1.0 - a1) / Math.Sin(φ);
                        sp = 0.5 * rho * Vlocal * Vlocal * BlDEle[j, i].SChord;
                    ...
                    BlDEle[j, i].DCd = cd;
                    BlDEle[j, i].DCm = cm;
                    BlDEle[j, i].DU = Vlocal;
                    BlDEle[j, i].DVx = v * (1.0 - a1);
                    BlDEle[j, i].DVy = v * (1.0 + b1);
                    BlDEle[j, i].DPx = test2[j];
                    BlDEle[j, i].DPy = test[j];
                    BlDEle[j, i].DTheta = theta;
                    BlDEle[j, i].DMz = cl * sp * BlDEle[j, i].SAeroCentJ2 - cd * sp * BlDEle[j, i].SAeroCentJ1 + cm * sp * BlDEle[j, i].SChord;
                    BlDEle[j, i].DMx = test[j] * BlDEle[j, i].SBlspan;
                    BlDEle[j, i].Vindx = v;
                    BlDEle[j, i].Vindy = ω * BlDEle[j, i].SBlspan;
                ...
                {
                    BlDEle[k, i].Torque = tor;
                    BlDEle[k, i].Thrust = tui;
                }

            }
            double Cp = Power / (0.5 * rho * Math.PI * TipRad * TipRad * v * v * v);
            double Ct_result = TuiLi / (0.5 * rho * Math.PI * TipRad * TipRad * v * v);
        ...
        /// <returns></returns>
        private double CalculateCp(double TSR, double Pit, T_ALAeroBladeElement[,] Bld, Airfoil1 Airfoil, double HubRad, double rho = 1.225, int Bldnum = 3)
        {
            (double Cp, _) = BEM(Bld, Airfoil, TSR, 10, HubRad, Pit, rho, Bldnum);
            return Cp; // 返回 Cp 值
        ...
        /// <param name="Bldnum"></param>
        /// <returns></returns>
        public Matrix<double> CalculateCp(double minTSR, double maxTSR, double TSRstp, ref T_ALAeroBladeElement[,] Bld, Airfoil1 Airfoil, double HubRad, double minPit = 0.0, double maxPit = 0.0, double Pitstp = 0.0, double rho = 1.225, int Bldnum = 3)
        {
            ...
            WriteTitle(directory, $"OPenhast 静态Cp 结果输出文件");
            ...
            for (int i = 0; i < TSRnum; i++)
            {
                for (int j = 0; j < Pitnum; j++)
                {
                    ...
                    Write(directory, Pit[j] * RadTodeg);
                    result[i, j] = CalculateCp(TSR[i], Pit[j], Bld, Airfoil, HubRad, rho, Bldnum);
                    WriteBEMEleValue();
                    WriteLine(directory);
        ...
        /// 第七列是P_out[kw]<br/>
        /// </returns>
        public Matrix<double> SteadyPowerCurveFunction(T_ALStp_pow std_pow, T_ALAeroBladeElement[,] Bld, Airfoil1 Airfoil, double HubRad, double rho = 1.225, int Bldnum = 3, bool show = true)
        {
            ...
            if (ifpitch)
            {
                ...
                for (int kkk = 0; kkk < data_length; kkk++)
                {
                    ...
                    double Cp = CalculateCp(λ, θ, Bld, Airfoil, HubRad, rho, Bldnum);
                    double P_out = Cp * KwMax;

                    if (P_out > opt_KW)
                    {
                        double fd(double x) => CalculateCp(λ, x, Bld, Airfoil, HubRad, rho, Bldnum) * KwMax - opt_KW;
                        θ = fzero(fd, θ, pitch_up, 1E-20, 200);
                    }
            ...
            {
                LogHelper.WarnLog("您将`ifpitch`设置为了`False`,这意味着风力机是定速和定桨距角的!从而不追求TSR");
        ...
        private (double Cp, double Ct) CalculateCpCt(double TSR, double windspeed, double Pit, ref T_ALAeroBladeElement[,] Bld, Airfoil1 Airfoil, double HubRad, double rho = 1.225, int Bldnum = 3)
        {
            (double Cp, double Ct) = BEM(Bld, Airfoil, TSR, windspeed, HubRad, Pit, rho, Bldnum);
        ...
        /// <returns></returns>
        private (double CpMax, double Tsr) FindCpOptAndTsropt(T_ALAeroBladeElement[,] Bld, Airfoil1 Airfoil, double HubRad, double rho = 1.225, int Bldnum = 3, double pit = 0.0)
        {
            double fd(double x) => (CalculateCp(x + 0.0000000001, pit, Bld, Airfoil, HubRad, rho, Bldnum) - CalculateCp(x, pit, Bld, Airfoil, HubRad, rho, Bldnum)) / 0.0000000001;
            double Tsr = fzero(fd, 0.5, 30, 1E-20);
            double cp = CalculateCp(Tsr, pit, Bld, Airfoil, HubRad, rho, Bldnum);
            return (cp, Tsr);
        ...
        private double UpdateInflowAngle(double theta, double Vx, double Vy, int FoilNo, Airfoil1 Airfoils, double TipRad, double HubRad, double BlSpn, double Solid, double Azimuth, double chi0)
        {
            ...
            if (Res(Math.PI / 2.0) * Res(ep) < 0)
            {
                x0 = ep;
                x1 = Math.PI / 2.0;
                phi = fzero(Res, x0, x1, 1E-15, 200, ref phiStore);
